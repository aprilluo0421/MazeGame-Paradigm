...
def run_trial(display, screen, trial_map, spr_player, spr_tiles, background, block_num, trial_num, maze_ID):
    ...
    class Player():
        ...
        def cal_4_sides(self):
            self.right_index = ((self.x+32) / 32, self.y / 32)
            self.left_index = ((self.x-32) / 32, self.y / 32)
            self.up_index = (self.x / 32, (self.y-32) / 32)
            self.down_index = (self.x / 32, (self.y+32) / 32)

            if self.right_index in hidden_block_index or self.x < screen_length-32:
                self.sides[1] = 1
            if self.right_index in brick_index or self.x >= screen_length-32:
                self.sides[1] = 0
            if self.right_index == target:
                self.sides[1] = 2
            if self.left_index in hidden_block_index or self.x > 0:
                self.sides[3] = 1
            if self.left_index in brick_index or self.x <= 0:
                self.sides[3] = 0
            if self.left_index == target:
                self.sides[3] = 2
            if self.up_index in hidden_block_index or self.y > 0:
                self.sides[0] = 1
            if self.up_index in brick_index or self.y <= 0:
                self.sides[0] = 0
            if self.up_index == target:
                self.sides[0] = 2
            if self.down_index in hidden_block_index or self.y < screen_width-32:
                self.sides[2] = 1
            if self.down_index in brick_index or self.y >= screen_width-32:
                self.sides[2] = 0
            if self.down_index == target:
                self.sides[2] = 2 

        def classify_blocks(self):
            block = ''
            if self.sides.count(1) == 2:
                block = 'N' # neutral path
            if self.sides.count(1) >= 3:
                sides_2nd = [0,0,0,0,0,0,0,0]
                ...
                if pos_0 in hidden_block_index or pos_0 == target or self.y - 32 > 0:
                    sides_2nd[0] = 1
                if pos_0 in brick_index or self.y - 32 <= 0:
                    sides_2nd[0] = 0
                if pos_1 in hidden_block_index or pos_1 == target or self.y > 0 or self.x < screen_length-32:
                    sides_2nd[1] = 1
                if pos_1 in brick_index or self.y <= 0 or self.x >= screen_length-32:
                    sides_2nd[1] = 0
                if pos_2 in hidden_block_index or pos_2 == target or self.x + 32 < screen_length-32:
                    sides_2nd[2] = 1
                if pos_2 in brick_index or self.x + 32 >= screen_length-32:
                    sides_2nd[2] = 0
                if pos_3 in hidden_block_index or pos_3 == target or self.y < screen_width-32 or self.x < screen_length-32:
                    sides_2nd[3] = 1
                if pos_3 in brick_index or self.y >= screen_width-32 or self.x >= screen_length-32:
                    sides_2nd[3] = 0
                if pos_4 in hidden_block_index or pos_4 == target or self.y + 32 < screen_width-32:
                    sides_2nd[4] = 1
                if pos_4 in brick_index or self.y + 32 >= screen_width-32:
                    sides_2nd[4] = 0
                if pos_5 in hidden_block_index or pos_5 == target or self.x > 0 or self.y < screen_width-32:
                    sides_2nd[5] = 1
                if pos_5 in brick_index or self.x <= 0 or self.y >= screen_width-32:
                    sides_2nd[5] = 0
                if pos_6 in hidden_block_index or pos_6 == target or self.x - 32 > 0:
                    sides_2nd[6] = 1
                if pos_6 in brick_index or self.x - 32 <= 0:
                    sides_2nd[6] = 0
                if pos_7 in hidden_block_index or pos_7 == target or self.y > 0 or self.x > 0:
                    sides_2nd[7] = 1
    ...
    start_loc = (1.0, 1.0)
    line = ['nav', block_num, trial_num, 'start', start_time, start_loc, 'P', maze_ID]
    output_df.append(line)
    ...
    while alive:
        ...
        blank_square = pygame.image.load("assets/black-square-" + str(square_size) + ".png").convert_alpha()
...
def run_guess(display, screen, trial_map, spr_player, spr_tiles, background, block_num, trial_num, maze_ID):
...
def black_square(surface,screen):
    # Windows version, this line got commented out
    surface.fill((0, 0, 0))
...
for set in range(len(maze_set)):
    ...
    for block in range(nRepeat):
        ...
        for j in range(len(rand_seq)):
            ...
            run_trial(display, screen, trial_map, spr_player, spr_tiles, background, block_num, j, maze_ID)
            display = pygame.display.set_mode((0,0,), pygame.FULLSCREEN)
            black_square(display,screen)
        ...
        for j in range(len(rand_seq)):
            ...
            run_guess(display, screen, trial_map, spr_player, spr_tiles, background, block_num, j, maze_ID)
            display = pygame.display.set_mode((0,0,), pygame.FULLSCREEN)
            black_square(display,screen)
